...
RSpec.describe RelatedWorkOrderJob, :db_connection, type: :job do
  ...
  it 'links work orders' do
    RelatedWorkOrderJob.new.perform(1, today, '00000001', ['00000002'])
  ...
  it "creates new note records" do
    RelatedWorkOrderJob.new.perform(23, today, '00000002', [])
  ...
  it 'ignores already processed notes' do
    ...
    RelatedWorkOrderJob.new.perform(23, 1.day.from_now, '00000003', ['00000004'])
  ...
  context 'when a work_order is not in the graph database' do
    it 'creates a new work order model if a note belongs to an unknown work order' do
      new_work_order = build :hackney_work_order, reference: '00000003', created: today
      expect(Hackney::WorkOrder).to receive(:find).with('00000003') { new_work_order }

      RelatedWorkOrderJob.new.perform('1', today, '00000001', ['00000003'])
    ...
    it "create a new work order model if the reference exists in the api" do
      ...
      RelatedWorkOrderJob.new.perform('1', today, '00000002', ['00000003'])
    ...
    it 'raises an exception if the work_order for a note is invalid' do
      ...
      expect {
        RelatedWorkOrderJob.new.perform(1, today, '00000003', ['00000002'])
